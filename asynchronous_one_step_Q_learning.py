...
# FINAL_EPSILONS = [0, 0, 0] # Final values of epsilon
# INITIAL_EPSILONS = [0, 0, 0] # Starting values of epsilon
...
def actorLearner(thread_id, target, lock):
    ...
    sess = tf.InteractiveSession(target)
    ...
    game_state = game.GameState(update_disp=thread_id == 0)
    ...
    print "THREAD ", thread_id, "STARTING...", "EXPLORATION POLICY => INITIAL_EPSILON:", INITIAL_EPSILON, ", FINAL_EPSILON:", FINAL_EPSILON
    ...
    while T < TMAX and score < WISHED_SCORE:
        ...
        if terminal:
            print "THREAD:", thread_id, "/ TIME", T, "/ TIMESTEP", t, "/ STATE", state, "/ EPSILON", epsilon, "/ ACTION", action_index, "/ REWARD", r_t, "/ Q_MAX %e" % np.max(readout_t), "/ SCORE", score
            score = 0
...
if __name__ == "__main__":
    ...
    for i in range(THREADS):
        t = threading.Process(target=actorLearner, args=(i, server.target, lock))
        threads.append(t)
